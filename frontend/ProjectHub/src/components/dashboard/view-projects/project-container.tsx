...
const ProjectsContainer = () => {
	...
	const filters: ProjectFilters = {
		...
		priority:
			(search.priority as keyof typeof PRIORITY_MAP) ||
			defaultProjectFilters.priority,
		status:
			(search.status as keyof typeof STATUS_MAP) ||
			defaultProjectFilters.status,
				...
				{(data?.totalCount ?? 0) > ITEMS_PER_PAGE && (
					<div className='mt-6 w-full flex justify-center'>
						<ShadcnPagination
							...
							onPageChange={(page) =>
								navigate({ to: dashboardRoute.to, search: { ...search, page } })
							}
						/>
					</div>
				)}
